...
public class NewsService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (connectionAvailable(getApplicationContext())) {
            RemoteViews views = new RemoteViews("com.hkp.widget", R.layout.new_app_widget);
            views.setTextViewText(R.id.tvTitle, "Busy retrieving data...");
            views.setTextViewText(R.id.tvDesc, "Busy retrieving data...");
    ...
    public class GetStoriesInBackground extends AsyncTask<Integer, Integer, String>
    {
        ...
        protected void onPostExecute(String result) {
            ...
            int randVal = random.nextInt(titles.size());

            RemoteViews views = new RemoteViews("com.hkp.widget", R.layout.new_app_widget);
            views.setTextViewText(R.id.tvTitle, titles.get(randVal));
            views.setTextViewText(R.id.tvDesc, descriptions.get(randVal));

            Uri uri = Uri.parse(links.get(randVal));
            Intent linkIntent = new Intent(Intent.ACTION_VIEW, uri);

            PendingIntent piLink = PendingIntent.getActivity(getApplicationContext(), 0, linkIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            views.setOnClickPendingIntent(R.id.tvDesc, piLink);

            PendingIntent piSync = PendingIntent.getService(getApplicationContext(), 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
            views.setOnClickPendingIntent(R.id.ivSync, piSync);
